...
def feedback_keyboard():
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("💬 Оставить отзыв (ТЫК)", url=REVIEW_LINK))
...
def callback_handler(call):
    ...
    bot.send_message(
        ...
        "🌟 <b>Добро пожаловать в Nairy Stars!</b>\n"
        "━━━━━━━━━━━━━━━━\n\n"
        "Nairy Stars — это магазин по продаже Telegram Stars. "
        "Мы предлагаем быстрые, качественные и выгодные цены.\n\n"
        "🎯 <i>Наша цель — сделать покупку Telegram Stars максимально простой, удобной и надёжной для каждого клиента.</i>\n\n"
        "Спасибо, что выбрали Nairy Stars! 🤍\n\n"
        "━━━━━━━━━━━━━━━━\n"
        "⭐ У нас более 1100 отзывов\n\n"
        "📢 Наш канал:\n"
        ...
        try:
            bot.edit_message_caption(
                caption=(
                    ...
                    f"✏️ Введите количество звёзд:\n"
                    f"<i>Минимум: {MIN_STARS} шт.</i>"
                ),
                chat_id=chat_id,
                message_id=message_id,
...
def process_text_input(message):
    ...
    if step == "ask_amount":
        ...
        if recipient == "me":
            recipient_line = "👤 Получатель: <b>Вы</b>"
        else:
            recipient_line = f"👤 Получатель: <b>Друг</b> (@{recipient_username})"
        ...
        bot.send_message(
            ...
            f"👤 Получатель: {BANK_NAME}\n\n"
            f"⚠️ <i>В сообщении при переводе укажите ваш юзернейм:</i>\n"
            f"<b>@{message.from_user.username or 'ваш_юзернейм'}</b>\n\n"
    ...
    if step == "ask_review_text":
        ...
        user_data.pop(user_id, None)
        stars_text = "⭐" * rating
        bot.send_message(message.chat.id, f"🌟 Спасибо за отзыв! {stars_text}", reply_markup=main_menu())
        return
